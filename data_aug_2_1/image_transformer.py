...
class SampleImgTransformer:
    def __init__(self, *, image, bg_color, bg_thresh, bboxes, classes):
        size = np.shape(image)
        self.height = size[0]
        self.width = size[1] 
    ...
    def perspectiveTransform(self, *, maxXangle, maxYangle, maxZangle, bgColor=255, bounding_boxes, class_nums, bkImg):
        ...
        #dstX = (M11*srcX + M12*srcY + M13) / (M31*srcX + M32*srcY + M33)
        #dstY = (M21*srcX + M22*srcY + M23) / (M31*srcX + M32*srcY + M33)
    ...
    def FusionBackground(self,*, bkg_img):
        ...
        m = process_img[start_y:end_y, start_x:end_x, :]
        i_a_not_m_1 = self.modified_image != -1
